...
if [ "$RECOVER" == 0 ];then
	...
	if [ "$2" == "emmc" ]; then
		...
		execute "cp ./tmp/rootfs/rootfs.tar /mnt/rootfs/"
		execute "sync"
		execute "umount /mnt/rootfs/"
	elif [ "$2" == "djv" ]; then
		execute "mount ${device}2 /mnt"
		execute "tar xvf ./tmp/rootfs/OK57xx-linux-fs.tar.bz2 -C /mnt"
		execute "sync"
		execute "umount /mnt"
	else
		execute "rm -rf /mnt/rootfs"
		execute "mkdir /mnt/rootfs"
		execute "mount ${device}2 /mnt/rootfs"
		execute "tar -xvf ./tmp/rootfs/rootfs.tar -C /mnt"
		execute "sync"
		execute "umount /mnt/rootfs/"
	fi
	echo "completed!"
