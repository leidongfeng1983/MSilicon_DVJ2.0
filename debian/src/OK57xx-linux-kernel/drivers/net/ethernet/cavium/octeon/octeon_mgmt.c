...
static void octeon_mgmt_adjust_link(struct net_device *netdev)
{
	...
	unsigned long flags;
	int link_changed = 0;
	pr_info("Bill Test octeon_mgmt_adjust_link start.\n");
	if (!phydev)
		return;
	pr_info("Bill Test octeon_mgmt_adjust_link finish.\n");
	...
	if (link_changed != 0) {
		if (link_changed > 0) {
			pr_info("%s: Bill Test Link is up - %d/%s\n", netdev->name,
				phydev->speed,
				phydev->duplex == DUPLEX_FULL ?
				"Full" : "Half");
		} else {
			pr_info("%s: Bill Test Link is down\n", netdev->name);
		}
	}
}
